 ...
 * Shared assets (progress.css, progress.js, demo.css, demo.js, og-image.png) stay at root
...
const ORIGIN = 'https://ghost-progress-plugin.greedylabs.kr';
...
const CDN = 'https://cdn.jsdelivr.net/gh/GreedyLabs/ghost-progress-plugin@1';
...
const REPO = 'https://github.com/GreedyLabs/ghost-progress-plugin';
...
function page(lang) {
  ...
  const i18nJson = JSON.stringify({ copy: t.copy, copyDone: t.copyDone });
  return `<!DOCTYPE html>
<html lang="${lang}">
<head>
    <meta charset="utf-8">
    ...
    <script>
      (function () {
        try {
          var m = localStorage.getItem('gprog-theme') || 'system';
...
${hreflangs(url)}
    ...
    <meta property="og:site_name" content="ghost-progress-plugin">
...
${jsonld(lang, t, url)}
    ...
    <script defer src="https://umami.greedylabs.kr/script.js"
            data-website-id="d8bbea54-9776-41bc-9c8a-ec6cc3381c72"></script>
</head>
<body>
    <aside class="panel">
        ...
        <div class="field">
            ...
            <select id="f-position">
                <option value="top" selected>${esc(t.optTop)}</option>
                <option value="bottom">${esc(t.optBottom)}</option>
            </select>
        </div>

        <div class="field check">
            <input type="checkbox" id="f-useThemeColor" checked>
            <label for="f-useThemeColor">${esc(t.lblAutoColor)}</label>
        </div>
        <div class="field">
            <label for="f-color">${esc(t.lblColor)}</label>
            <input type="color" id="f-color" value="#1a73e8" disabled>
        </div>

        <div class="field row">
            <div><label for="f-height">${esc(t.lblHeight)}</label><input type="number" id="f-height" value="4" step="1" min="1"></div>
            <div><label for="f-zindex">${esc(t.lblZindex)}</label><input type="number" id="f-zindex" value="100" step="10"></div>
        </div>
    ...
    <section class="demo-comments">
        ...
        <script src="https://giscus.app/client.js"
                ...
                data-category="ghost-progress-plugin.greedylabs.kr"
                data-category-id="DIC_kwDOTHbOr84DAePj"
                data-mapping="specific"
                data-term="ghost-progress-plugin"
    ...
    <script src="/progress.js" data-auto="false"></script>
    <script>window.GPROG_I18N = ${i18nJson};</script>
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>ghost-progress-plugin: Reading progress bar for Ghost</title>
