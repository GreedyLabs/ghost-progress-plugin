...
const HINT = { fontWeight: 400, color: 'var(--ui-muted)' };
// A color field accepts any CSS color: hex, named, or a token like var(--x).
// The swatch picker only understands hex, so fall back when the value isn't one.
const hexOr = (v, d) => (/^#[0-9a-fA-F]{3,8}$/.test((v || '').trim()) ? v.trim() : d);

const INIT = {
  content: '.gh-content', position: 'top', height: DEF.height,
  color: '', zIndex: DEF.zIndex, anchor: '', nav: false
};
...
export default function Configurator({ locale }) {
  ...
  // The nav toggle both shows the demo nav and points the anchor at it.
  const toggleNav = (e) => {
    const on = e.target.checked;
    setO((p) => ({ ...p, nav: on, anchor: on ? '.demo-nav' : (p.anchor === '.demo-nav' ? '' : p.anchor) }));
  };
  ...
  useEffect(() => {
    ...
    inst.current = window.GreedyLabsGhostProgress.create({
      ...
      color: o.color.trim() || undefined, zIndex: num(o.zIndex),
      anchor: o.anchor.trim() || undefined
    });
  ...
  return (
    <>
      <aside className="panel">
        ...
        <div className="field">
          <label htmlFor="f-content">{t('lblContent')} <span style={HINT}>{t('hintContent')}</span></label>
        ...
        <div className="field">
          <label htmlFor="f-color">{t('lblColor')} <span style={HINT}>{t('hintColor')}</span></label>
          <div className="color-input">
            <input type="text" id="f-color" value={o.color} onChange={set('color')} placeholder="var(--ghost-accent-color)" />
            <input type="color" aria-label={t('lblColor')} value={hexOr(o.color, '#1a73e8')} onChange={set('color')} />
          </div>
        </div>
        <div className="field row">
        ...
        <div className="field check">
          <input type="checkbox" id="f-nav" checked={o.nav} onChange={toggleNav} />
          <label htmlFor="f-nav">{t('lblNav')}</label>
        </div>
        <div className="field">
          <label htmlFor="f-anchor">{t('lblAnchor')} <span style={HINT}>{t('hintAnchor')}</span></label>
          <input type="text" id="f-anchor" value={o.anchor} onChange={set('anchor')} placeholder=".site-nav" />
        </div>
      ...
      {o.nav ? <div className="demo-nav">Demo nav</div> : null}
    </>
  );
}
...
function buildSnippet(o) {
  ...
  if (o.color.trim()) a.push(`data-color="${esc(o.color.trim())}"`);
  if (ni(o.zIndex) !== DEF.zIndex) a.push(`data-z-index="${ni(o.zIndex)}"`);
  if (o.anchor.trim()) a.push(`data-anchor="${esc(o.anchor.trim())}"`);
